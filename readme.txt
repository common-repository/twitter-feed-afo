=== Recent Tweet Widget AFO ===
Contributors: avimegladon
Donate link: https://www.aviplugins.com/donate/
Tags: twitter, widget, twitter widget, twitter feed, twitter sidebar, twitter sidebar widget, feed, recent, recent tweet, tweet, tweet feed
Requires at least: 2.0.2
Tested up to: 5.9.3
Stable tag: 2.2.9
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This is a recent twitter feed widget. It's compatible with twitter api v1.1. so no warries. :) 

== Description ==

* This is a simple twitter feed widget. 
* just install the plugin and add the twitter feed widget in the sidebar. 
* add twitter credentials data in `Settings-> Tweet Widget Settings` and you are good to go. 
* Add css as you prefer because the feed display structure is really very simple.

= Other =
* This plugin supports twitter api v1.1 so no worries. 

= Important =
* You need to create a new twitter app to get this plugin working.
* Please log in to <a href="https://apps.twitter.com/">https://apps.twitter.com/</a> with your twitter account Username and Password. And click on the Create new app button.
* A new window will open. Enter all the relevant data, and follow the instructions. Your new app will be created.
* Now on the API Keys tab you will get all the data required for the plugin.
* For Access Token and Access Token Secret
* Click the Generate Access Token button and your access token and key will be created. 
* You need to enter all these data at the plugin's settings page.

> Get a <a href="https://www.aviplugins.com/get-quote.php">quote</a> for creating your own custom plugin.

== Installation ==


1. Upload `recent_tweet_afo.zip` to the `/wp-content/plugins/` directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Go to `Settings-> Tweet Widget Settings`, and twitter credentials. It's really easy.
4. Go to `Appearance->Widgets` ,in available widgets you will find Twitter Widget AFO` widget, drag it to chosen widget area where you want it to appear.
5. Now visit your blog and you will see a working twitter feed section.


== Frequently Asked Questions ==

= For all kind of issues about this plugin =

1. Please email me at demoforafo@gmail.com. Or submit your query at https://www.aviplugins.com/support.php

== Screenshots ==

1. settings page view
2. widget view


== Changelog ==

= 2.2.9 =
* Improvements.

= 2.2.8 =
* .POT file added in languages folder for easy translation.

= 2.2.7 =
* Bug fixed.

= 2.2.6 =
* Some design modifications are made in plugin settings section.

= 2.2.5 =
* Plugin will work much faster now.

= 2.2.4 =
* File structure updated. Feed display updated. Shortcode added.

= 2.2.3 =
* Bug fixed.

= 2.2.2 =
* Message display updated.

= 2.2.1 =
* Plugin code updated.

= 2.2.0 =
* Plugin code updated.

= 2.1.0 =
* Code updated with some security modifications.

= 2.0.0 =
* Notice message bug fixed.

= 1.1.3 =
* admin menu related bug fixed.

= 1.1.2 =
* Fixed some minor bugs.

= 1.0.2 =
* warning message error is solved when blank value is returned from twitter.

= 1.0.1 =
* this is the first release.


== Upgrade Notice ==

= 1.0 =
I will update this plugin when ever it is required.
